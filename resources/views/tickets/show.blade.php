                                ...
                                >
                                    @foreach ($ticket->replies as $reply)
                                        @if (!$reply->is_internal || auth()->user()->isAgent() || auth()->user()->isAdmin())
                                            <div class="flex gap-4 {{ $reply->user_id === request()->user()->id ? 'flex-row-reverse' : '' }}">
                                                <div class="flex-shrink-0">
                                                    <div class="w-10 h-10 rounded-full bg-gradient-to-br from-gray-100 to-gray-200 border border-gray-300 flex items-center justify-center shadow-sm">
                                                        <span class="text-gray-700 font-bold text-sm">{{ strtoupper(substr($reply->user->name, 0, 1)) }}</span>
                                                    </div>
                                                </div>

                                                <div class="flex flex-col max-w-[85%] {{ $reply->user_id === request()->user()->id ? 'items-end' : 'items-start' }}">
                                                    <div class="flex items-center gap-2 mb-1">
                                                        <span class="text-sm font-bold text-gray-800">{{ $reply->user->name }}</span>
                                                        <span class="text-[11px] text-gray-400 font-medium">{{ $reply->created_at->diffForHumans() }}</span>
                                                    </div>

                                                    @if ($reply->is_internal)
                                                        <div class="p-4 rounded-2xl shadow-sm text-sm leading-relaxed {{ $reply->user_id === request()->user()->id ? 'bg-red-600 text-white rounded-tr-none' : 'bg-red-600 text-white rounded-tl-none' }}">
                                                            {{ $reply->message }}
                                                        </div>
                                                    @else
                                                        <div class="p-4 rounded-2xl shadow-sm text-sm leading-relaxed {{ $reply->user_id === request()->user()->id ? 'bg-indigo-600 text-white rounded-tr-none' : 'bg-gray-100 text-gray-800 rounded-tl-none border border-gray-200' }}">
                                                            {{ $reply->message }}
                                                        </div>
                                                    @endif
                                                </div>
                            ...
                            <div class="p-6 bg-gray-50 border-t border-gray-200">
                                <form action="{{ route('tickets.replies.store', $ticket) }}" method="POST" class="space-y-4">
                                    ...
                                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                            ...
                                            @if (auth()->user()->isAgent() || auth()->user()->isAdmin())
                                                <label class="inline-flex items-center cursor-pointer group">
                                                    <div class="relative">
                                                        ...
                                                        <div class="w-5 h-5 bg-white border-2 border-gray-300 rounded peer-checked:bg-indigo-600 peer-checked:border-indigo-600 transition-all"></div>
                                                        <svg class="absolute w-3.5 h-3.5 text-white top-0.5 left-0.5 opacity-0 peer-checked:opacity-100 transition-opacity" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="4">
                                                            <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                                                        </svg>
                                                    </div>
                                                    <span class="ml-3 text-sm font-medium text-gray-600 group-hover:text-gray-800 transition-colors">Internal Note</span>
                                                </label>
                                                @error('is_internal')
                                                <x-input-error messages="{{ $message }}"></x-input-error>
                                                @enderror
                                        ...
                                        <button type="submit" class="rounded-md bg-indigo-600 px-3 py-2 text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600">Post Comment</button>
                                    </div>
                                </form>
                            </div>
                        </div>
